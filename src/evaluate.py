...
def main() -> None:
    ...
    # Global accumulators for detailed metrics
    all_bag_targets = []
    all_bag_probs = []
    all_inst_targets = []
    all_inst_probs = []
    ...
    for pcap_path in pcap_files:
        ...
        if "y_bags" in res:
            all_bag_targets.extend(res["y_bags"])
            # bag_logits are logits, need sigmoid for probs
            bag_probs = torch.sigmoid(torch.tensor(res["bag_logits"])).numpy()
            all_bag_probs.extend(bag_probs)

        if "y_insts" in res:
            for y_inst, inst_logit in zip(res["y_insts"], res["inst_logits"]):
                all_inst_targets.extend(y_inst)
                inst_probs = 1.0 / (1.0 + np.exp(-inst_logit))
                all_inst_probs.extend(inst_probs)
    ...
    if all_bag_targets:
        print("\n" + "="*40)
        print("DETAILED BAG METRICS")
        print("="*40)
        y_true = np.array(all_bag_targets)
        y_score = np.array(all_bag_probs)
        y_pred = (y_score >= 0.7).astype(int)
        
        print("Confusion Matrix:")
        print(metrics.confusion_matrix(y_true, y_pred))
        print("\nClassification Report:")
        print(metrics.classification_report(y_true, y_pred, target_names=["Negative", "Positive"], digits=4))
    ...
    if all_inst_targets:
        print("\n" + "="*40)
        print("DETAILED INSTANCE METRICS")
        print("="*40)
        y_true = np.array(all_inst_targets)
        y_score = np.array(all_inst_probs)
        y_pred = (y_score >= 0.7).astype(int)
